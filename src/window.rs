...
fn manual_update_required_message(language: LanguageId, version: &str) -> String {
    match language {
        LanguageId::English => format!("Version v{version} is available, but the program or release asset name has changed. Automatic update is unavailable. Open GitHub Releases and update manually."),
        LanguageId::Dutch => format!("Versie v{version} is beschikbaar, maar de programma- of releasebestandsnaam is gewijzigd. Automatisch bijwerken is niet mogelijk. Open GitHub Releases en werk handmatig bij."),
        LanguageId::Spanish => format!("La versión v{version} está disponible, pero el nombre del programa o de los archivos de la versión ha cambiado. La actualización automática no está disponible. Abre GitHub Releases y actualiza manualmente."),
        LanguageId::French => format!("La version v{version} est disponible, mais le nom du programme ou des fichiers de publication a changé. La mise à jour automatique est indisponible. Ouvrez GitHub Releases et mettez à jour manuellement."),
        LanguageId::German => format!("Version v{version} ist verfügbar, aber der Programmname oder der Name der Release-Datei hat sich geändert. Die automatische Aktualisierung ist nicht möglich. Öffnen Sie GitHub Releases und aktualisieren Sie manuell."),
        LanguageId::Japanese => format!("v{version} を利用できますが、プログラム名またはリリースファイル名が変更されています。自動更新できません。GitHub Releases を開き、手動で更新してください。"),
        LanguageId::Korean => format!("v{version} 버전을 사용할 수 있지만 프로그램 이름 또는 릴리스 파일 이름이 변경되었습니다. 자동 업데이트를 사용할 수 없습니다. GitHub Releases를 열어 수동으로 업데이트하세요."),
        LanguageId::SimplifiedChinese => format!("检测到 v{version}，但程序名称或发布文件名称已发生变化，无法自动升级。请前往 GitHub Releases 手动更新。"),
        LanguageId::TraditionalChinese => format!("偵測到 v{version}，但程式名稱或發佈檔案名稱已變更，無法自動升級。請前往 GitHub Releases 手動更新。"),
        LanguageId::Russian => format!("Доступна версия v{version}, но название программы или файла релиза изменилось. Автоматическое обновление недоступно. Откройте GitHub Releases и обновитесь вручную."),
        LanguageId::PortugueseBrazil => format!("A versão v{version} está disponível, mas o nome do programa ou do arquivo da versão mudou. A atualização automática não está disponível. Abra o GitHub Releases e atualize manualmente."),
    }
}
                    ...
                    updater::UpdateUiResult::ManualUpdateRequired { version } => {
                        let (strings, language) = {
                            ...
                            state
                                ...
                                .unwrap_or_else(|| (LanguageId::English.strings(), LanguageId::English))
                        };
                        tray_icon::clear_notification(hwnd);
